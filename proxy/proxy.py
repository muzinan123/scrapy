...
from webapp.scrapy_model import Proxy as Pro
...
class Server(Connection):
    ...
    def connect(self):
        ...
        print 'proxy', proxy
        if proxy:
            logging.debug('use random proxy %s:%d' % (proxy.ip, proxy.port))
            self.proxy = proxy
            self.conn = socks.socksocket()
            self.conn.set_proxy(getattr(socks, proxy.protocal, socks.HTTP),
                                proxy.ip, proxy.port)
        else:
